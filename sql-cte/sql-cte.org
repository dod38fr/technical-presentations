...
- Fait partie du SQL, langage d'interrogation de base de donnée relationelle
...
#+begin_src plantuml :file tree.png
  @startuml
  (m1) --> (m1.1)
  (m1) --> (m1.2)
  (m1.1) --> (m1.1.1)
...
- les msg_id représentent un chemin (/path/) pour faciliter la
  compréhension
...
#+begin_src sqlite :db ht-cte.db :colnames yes
  select * from message
#+end_src
...
#+begin_src sqlite :db ht-cte.db :colnames yes
   ...
   -- étape 1
     ...
     join tmp_msg where a.id = tmp_msg.parent_id -- 🎯 la récursion est là
...
** CTE récursive - nettoyage

#+begin_src sqlite :db ht-cte.db :colnames yes
  with recursive tmp_msg(message_key, parent_id) as (
    select message_key, parent_id from message where message_key = "m1.1.1"
    union all
    select a.message_key, a.parent_id from message as a
    join tmp_msg on a.id = tmp_msg.parent_id
  )
  select message_key from tmp_msg where parent_id isnull
#+end_src
...
- Gain de performance en évitant les allers et retours entre le service
  et la DB
